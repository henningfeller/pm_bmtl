...
plotPost = function( paramSampleVec , cenTend=c("mode","median","mean")[1] , 
										 compVal=NULL, ROPE=NULL, credMass=0.95, HDItextPlace=0.2, 
	...
	if ( is.null(col) ) col=leuphanaPalette$theme$red
	if ( is.null(border) ) border=leuphanaPalette$theme$grey
	...
	if ( cenTend=="mode" ){ 
		text( mo , cenTendHt ,
					bquote(mode==.(signif(mo,3))) , adj=c(.5,-2.5) , cex=cex )
	}
	if ( cenTend=="median" ){ 
		text( med , cenTendHt ,
					bquote(median==.(signif(med,3))) , adj=c(.5,-2.5) , cex=cex , col=cvCol )
	}
	if ( cenTend=="mean" ){ 
		text( mn , cenTendHt ,
					bquote(mean==.(signif(mn,3))) , adj=c(.5,-2.5) , cex=cex )
	}
	...
	# Display the HDI.
	lines( HDI , c(0,0) , lwd=4 , lend=1 , col = "white")
	text( mean(HDI) , 0 , bquote(.(100*credMass) * "% HDI" ) ,
				adj=c(.5,-1.7) , cex=cex , col = "white")
	text( HDI[1] , 0 , bquote(.(signif(HDI[1],3))) ,
				adj=c(HDItextPlace,-0.5) , cex=cex , col = "white")
	text( HDI[2] , 0 , bquote(.(signif(HDI[2],3))) ,
				adj=c(1.0-HDItextPlace,-0.5) , cex=cex, col = "white" )
	par(xpd=F)
	#
	return( postSummary )
}